...
namespace Banzai.Utility
{
    ...
    internal static class Guard
    {
        ...
        public static void AgainstNullProperty<TProperty>(string propertyName, TProperty property)
            where TProperty : class
        {
            if (property == null)
                throw new NullReferenceException(string.Format(CultureInfo.InvariantCulture, "{0} is null.",
                    propertyName));
        }
        ...
        public static void AgainstNullArgumentProperty<TProperty>(string parameterName, string propertyName,
            TProperty argumentProperty)
        {
        ...
        public static void AgainstNullOrEmptyArgument(string parameterName, string argument)
        {
            if (string.IsNullOrEmpty(argument))
                throw new ArgumentNullException(parameterName,
                    string.Format(CultureInfo.InvariantCulture, "{0} is null or empty.", parameterName));
        }
    }
}