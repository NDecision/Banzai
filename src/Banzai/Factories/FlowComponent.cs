...
namespace Banzai.Factories
{
    ...
    {
        private List<FlowComponent<T>> _children = new();
        private Type _shouldExecuteBlockType;
        ...
        /// </summary>
        [DataMember]
        public Type ShouldExecuteBlockType
        {
            get => _shouldExecuteBlockType;
            set
            {
                if (!typeof(IShouldExecuteBlock<T>).IsAssignableFrom(value))
                    throw new ArgumentException("blockType must be assignable to IShouldExecuteBlock<T>.",
                        nameof(value));
                _shouldExecuteBlockType = value;
            }
        }
